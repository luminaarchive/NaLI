...
export function ConversationThread({ messages }: ConversationThreadProps) {
  // Only show follow-up exchanges (after the initial prompt+report)
            ...
            {msg.role === "user" ? (
              ...
              <div className="flex flex-col items-start">
                <div className="flex items-center gap-2 mb-2">
                  <span className="flex h-5 w-5 items-center justify-center rounded-full bg-[#00FFB3]/15 text-[10px] font-bold text-[#00FFB3]">N</span>
                ...
                <div className="w-full rounded-xl border border-white/[0.05] bg-white/[0.02] px-4 py-4 prose prose-invert prose-sm max-w-none nali-thread-content">
                  ...
                  `}</style>
                  {/* Show streaming cursor if last message is empty (being streamed) */}
                  {msg.content === "" ? (
