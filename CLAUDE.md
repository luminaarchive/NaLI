...
### 28 Juni 2026 (lanjutan 2), Bucket B Step 2.2 Living Articles + 2.3 Citizen Science -> Checkpoint 2 (founder-directed)

- **Step 2.2 Living Articles (live, push `61906ac`)**: field opsional author-declared
  di `Article` (`articleStatus: terkini|perlu-tinjauan|digantikan`, `lastVerified`,
  `supersededBy`) + `ARTICLE_CURRENCY_LABEL`; diparse di `content.ts`. **Currency
  TIDAK pernah ditebak dari umur** (anti-spekulasi), hanya yang penulis nyatakan;
  kalau kosong cuma tampil tanggal. Mengisi **seam Fase 2** di baris STATUS
  KnowledgeGenome (badge currency, "diperiksa {lastVerified}", "digantikan oleh
  {artikel}", "{n} catatan riwayat & koreksi -> #riwayat"). Section `#riwayat`
  menyatukan `changelog[]` + koreksi artikel (klaim lama->baru + alasan + sumber)
  jadi satu timeline. Backward-compatible (49 artikel render tanpa perubahan).
  Diverifikasi live dengan seed sementara di Samalas lalu di-revert.
- **Step 2.3 Citizen Science Bridge (live, push `02dfa33`)**: founder OK perluas
  scope (terima kiriman publik + host foto), **laporan PRIVAT sampai dipromosikan**
  manual (observasi belum terverifikasi tak pernah jadi klaim publik; lindungi dari
  poaching/PII/defamasi). Migrasi `0008` `citizen_reports` (anon insert-only +
  CHECK panjang sebagai guard spam; admin via `private.is_admin()`) + bucket
  `citizen-reports` (5MB, mime gambar) via Supabase MCP, advisor bersih.
  `/api/report` (same-origin, validasi, honeypot, GPS sanity, upload foto, insert
  pakai anon key seperti /api/subscribe). `ReportForm` di `/misi` (Geolocation
  opt-in, foto, progressive-enhancement, copy "belum terverifikasi" eksplisit).
  `/admin/reports` triase (flip status, tautan peta OSM, "salin ke draf"; promosi
  tetap tindakan editorial manual). **PHASE-3 SEAM** di `/admin/reports` untuk
  "Misi Verifikasi Lapangan" dari Lab (Bucket C). Diverifikasi live E2E (submit
  browser + curl upload foto -> row privat; /admin/reports 307) lalu test row +
  foto + temp policy dibersihkan total.
- **Koordinasi sesi paralel**: rebase 2 kali di atas fix RAG paralel (route 503 +
  log `5e4783e`; model embeddings `gemini-embedding-001` `0f7ed4d`). **Selaraskan
  `detect-contradictions.mjs` ke `gemini-embedding-001` (768d)** agar satu model
  embedding seluruh repo (push `b51a628`). Flag keamanan: `tmp_ingest_embedding`
  SECURITY DEFINER bisa dipanggil anon (warisan kerja RAG paralel) -> diparkir
  sebagai chip tugas founder.
- **check:editorial** sempat 2x FAIL em-dash (label select + judul admin) ->
  dibersihkan sebelum push ([[no-em-dashes]] sekali lagi terbukti berharga).
- **Status kunci Gemini (blocker tunggal yang tersisa)**: jawaban RAG tutor +
  `npm run detect:contradictions` butuh `GOOGLE_GENERATIVE_AI_API_KEY` (tugas
  founder, chip ada); sampai diset, contradiction engine = kosong (aman).
- **Sisa roadmap**: hanya **Bucket C** (Internal Intelligence Lab, ter-autentikasi:
  `/lab`, crawler GBIF/iNat/OSINT, Lazarus score, Ghost Signals). Semua seam Fase 3
  sudah terpasang (reading-paths + /admin/reports). Bucket C butuh percakapan desain
  tersendiri (anti-spekulasi + auth + infra crawler) sebelum kode. Lihat
  [[reader-intelligence-roadmap]].

### 29 Juni 2026, Bucket C Step 3.2: Lab harvesters (GBIF + iNat + IUCN) (founder-directed)

Lanjut Phase 3. Step 3.1 (foundation: `/lab` gated, `lab_leads` RLS admin-only,
CI import guard) sudah dipush sesi lalu (`708a199`). Step 3.2 = harvester yang
mengisi `lab_leads`. Push `8dc01f5`.

- **Investigasi API (terverifikasi live):** GBIF `api.gbif.org/v1` TANPA auth
  (`country=ID` = 5,46 jt okurensi, facet=year jalan); iNaturalist
  `api.inaturalist.org/v1` TANPA auth untuk baca (`place_id=6966` = Indonesia,
  terkonfirmasi); IUCN Red List **v4** (`api.iucnredlist.org/api/v4`,
  `Authorization: Bearer <token>`) **butuh token** (403 tanpa) -> tugas founder.
- **Arsitektur pemisahan (penting):** harvester READ-ONLY ke API + tulis HANYA ke
  `content/lab-raw/` (gitignored). Satu-satunya langkah privileged = `build-leads.mjs`
  yang baca dump lalu upsert ke Supabase pakai service-role key. Jadi harvester
  bisa di-dry-run TANPA kredensial; hanya tulis DB yang butuh key (fail-closed).
- **File** (`scripts/lab/`): `_shared.mjs` (fetchJson UA+backoff, dump IO, lazy
  service-role client null tanpa key), `seed-taxa.mjs` (12 taksa dari klaster
  Lazarus NaLI sendiri + taksa still-lost yang sudah dikutip; TIDAK scan 5,4jt
  buta), `harvest-gbif.mjs` (match usageKey -> histogram tahun: last record, total/
  ID count, bucket per dekade, tren 20thn), `harvest-inat.mjs` (research-grade last
  obs + count; **match nama ilmiah EKSAK saja**), `harvest-iucn.mjs` (enrichment;
  **no-op bersih tanpa IUCN_API_TOKEN**), `build-leads.mjs` (gabung dump -> baris
  `lab_leads`; `signals` = bukti mentah ternormalisasi 0..1 + catatan bersumber,
  **TANPA score** itu Step 3.3; upsert onConflict taxon_name **tak pernah menimpa
  status/notes** manusia; `--dry-run` jalan tanpa key).
- **Bug ditangkap saat dry-run:** iNat fuzzy-match menempelkan 7734 obs taksa lain
  ke Gallirallus sharpei (taksa ragu yang tak ada di iNat) -> diperketat ke match
  EKSAK; false-negative (no match) jauh lebih aman daripada false-positive yang
  menurunkan lead lost sungguhan. Setelah fix: Sharpe rail NO MATCH (benar).
- **Hasil dry-run nyata (12 taksa):** sinyal jujur, contoh Zaglossus attenboroughi
  gap 65thn, Varanus zugorum 46thn, Cyornis ruckii & Gallirallus sharpei 0 rekaman
  (lost sejati) vs spesies refound dgn rekaman terbaru. IUCN no-op (token belum ada).
- **Scaffolded `.env.local`:** placeholder `SUPABASE_SERVICE_ROLE_KEY` +
  `IUCN_API_TOKEN` (kosong; founder isi dari dashboard Supabase + apply token IUCN).
  Tak bisa kufabrikasi kredensial; harvester fail-closed sampai diisi.
- **npm scripts:** `lab:harvest:gbif|inat|iucn`, `lab:build-leads`. Gate hijau
  (tsc/lint/check:editorial/check:lab-isolation/build exit 0). Lihat
  [[reader-intelligence-roadmap]] + `docs/lab-architecture.md`.
- **Berikutnya (tunggu approval):** Step 3.3 Lazarus Score (formula transparan
  berbobot dari signals) + dashboard `/lab` dengan breakdown; lalu 3.4 misi ->
  DB + promosi 1-klik lead -> "Misi Verifikasi Lapangan", 3.5 Ghost Signals.
  **Blocker run nyata:** SUPABASE_SERVICE_ROLE_KEY (build-leads) + IUCN_API_TOKEN
  (enrichment) = tugas founder.

### 29 Juni 2026 (lanjutan 3), Bucket C Step 3.5: Ghost Signals + CHECKPOINT 3 SELESAI (founder-directed)

Mode otonomi penuh. Step terakhir Bucket C. Push `c6ea410`.

- **Rekonsiliasi infra:** `watch_alerts`/`alert_events` (dari kerja paralel) DIPERIKSA
  ternyata fitur LAIN (notifikasi perubahan konten internal: topic_slug/series_id/
  article_slug/trigger_type), bukan Ghost Signals. Memaksa sinyal eksternal ke
  `alert_events` (butuh parent watch_alerts via FK NOT NULL) = salah fit. Jadi
  dibiarkan utuh, dibuat tabel khusus `ghost_signals` (itulah rekonsiliasi jujurnya).
- **Migrasi `ghost_signals` (0012 via MCP):** source (youtube/xeno-canto/inaturalist),
  external_id, title, url, observed_on, location_label, taxon_hint, summary, score,
  signals jsonb, provenance (api/curated/sample), status (signal/investigating/
  promoted/dismissed). RLS admin-only (tanpa anon), unique(source,external_id). Advisor
  bersih.
- **Monitor (`scripts/lab/ghost/`):** `harvest-inat-needsid` (NYATA tanpa key:
  needs_id place_id 6966, filter "belum teridentifikasi ke spesies" = rank kasar,
  38rb obs), `harvest-xenocanto` (v2 MATI -> v3 butuh XENOCANTO_API_KEY; fallback
  sample berlabel), `harvest-youtube` (butuh YOUTUBE_API_KEY; fallback sample),
  `build-signals` (skor kemenarikan transparan: misteri 0,45 + kebaruan 0,35 +
  dapat-dilacak 0,20; upsert ghost_signals tak timpa status/notes; --dry-run +
  --emit-sample). Snapshot nyata `lib/lab/sample-signals.ts` (16 sinyal: iNat nyata
  + 2 CONTOH). npm: `lab:ghost:inat|xenocanto|youtube`, `lab:build-signals`.
- **`/lab/signals`** (`app/lab/signals/page.tsx` + client `GhostSignalsBoard`):
  banner "INTERNAL, ANOMALI BELUM TERVERIFIKASI", filter sumber, rincian skor + tautan
  sumber asli + tombol "Promosikan ke Misi". `lib/lab/ghost.ts` getGhostSignals
  (DB->sample, dilabel CONTOH). Tautan dua arah /lab <-> /lab/signals.
- **Promosi seam diperluas:** `/api/lab/promote` kini terima `leadId` ATAU `signalId`.
  Dari sinyal -> misi "Verifikasi lapangan: X" dengan framing lebih hati-hati
  ("sinyal belum terverifikasi dan bukan klaim... pertanyaan terbuka..."), flip
  ghost_signals.status=promoted. Tetap PERTANYAAN, bukan klaim.
- **Verifikasi browser (Claude Preview, dev-bypass):** /lab/signals tampil, top sinyal
  iNat (Katydids 86, Softshell Turtles 82) + CONTOH xeno/youtube; expand rincian skor;
  klik promote -> sukses -> /misi misi "Verifikasi lapangan: Katydids..." badge
  "Berasal dari investigasi Lab" + "bukan klaim" + 0 kontributor. Gate hijau
  (tsc/lint/check:editorial/check:lab-isolation/build).
- **CHECKPOINT 3 (Bucket C) SELESAI.** Lab lengkap. Arsitektur ringkas:
  HARVEST (GBIF/iNat/IUCN + iNat-needsID/Xeno-canto/YouTube, read-only -> content/
  lab-raw gitignored) -> SKOR (Lazarus `lib/lab/scoring.ts` + kemenarikan ghost,
  transparan, HEURISTIK bukan probabilitas) -> DASHBOARD PRIVAT (`/lab` lead +
  `/lab/signals` ghost, admin-gated, fallback DB->sample berlabel CONTOH) -> GERBANG
  MANUSIA (promosi 1-klik) -> SATU artefak yang ke publik = MISI (pertanyaan, bukan
  klaim) -> /misi -> lapor warga (ReportForm, mission_id menaut). Tiga lapis isolasi:
  RLS admin-only (DB), `check:lab-isolation` (build gagal kalau kode publik impor
  lab/*), kontrak promosi satu arah. Blocker run nyata SEMUA opsional (ada fallback):
  SUPABASE_SERVICE_ROLE_KEY, IUCN_API_TOKEN, XENOCANTO_API_KEY, YOUTUBE_API_KEY,
  GOOGLE_GENERATIVE_AI_API_KEY (RAG+kontradiksi).

### 29 Juni 2026 (lanjutan 2), Bucket C Step 3.4: promosi lead -> Misi (loop umpan-balik) + mendekati Checkpoint 3 (founder-directed)

Mode otonomi penuh tetap aktif. Step 3.4 menyambung tombol "Promosikan ke Misi"
(disabled di 3.3) jadi hand-off Lab -> publik yang nyata. Push `71446b6`.

- **Migrasi `missions` (0011, via Supabase MCP):** tabel `public.missions` (id slug,
  title, description, status active/closed, **source editorial/lab**, **lead_id FK
  -> lab_leads**, evidence_needed jsonb, progress, timestamps). RLS: publik baca,
  admin tulis (`private.is_admin()`). Advisor bersih (tak ada isu baru; `missions`
  punya kedua policy). `citizen_reports.mission_id` (text) kini bisa menaut ke
  mission slug.
- **Data layer DB + fallback (`lib/missions.ts` jadi async):** baca DB (publik) ->
  seed JSON editorial -> (non-produksi) `content/missions/_lab-dev/` (gitignored).
  Dedup per id, DB menang. Status DB active/closed dipetakan ke aktif/selesai. JSON
  seed menjamin /misi tak pernah kosong walau DB mati. `types/missions.ts` +
  `source`/`leadId`. `/misi` jadi async + render badge "Berasal dari investigasi Lab"
  saat source=lab + anchor `#misi-<id>`.
- **Promosi (`app/api/lab/promote/route.ts`, di bawah app/api/lab = lab path):**
  same-origin + auth (admin via `is_current_user_admin`, atau dev-bypass).
  Ambil lead via `getScoredLeads` (fallback sample), **generate PERTANYAAN** ("Cari
  bukti terkini: X. ... Ini bukan klaim bahwa spesies masih ada atau sudah punah,
  melainkan pertanyaan terbuka...") + 4 butir bukti. **Tak pernah klaim.** Prod:
  upsert ke `missions` (idempotent per slug) + set lead status promoted. Dev-bypass:
  tulis JSON ke `_lab-dev` (fs), agar flow bisa kuverifikasi tanpa service key.
  Isolation aman: `lib/missions.ts` (publik) TIDAK impor lab; route promosi yang
  impor lab ada di app/api/lab.
- **UI:** tombol "Promosikan ke Misi" di `LabLeadsBoard` aktif -> POST -> state
  sukses "Misi dibuat" + tautan "Lihat misi publik". 
- **Verifikasi browser (Claude Preview, dev-bypass, Playwright sudah disconnect):**
  klik promote Zaglossus (skor 93) -> sukses -> /misi menampilkan misi baru dgn
  badge "Berasal dari investigasi Lab", judul pertanyaan, deskripsi "bukan klaim...",
  4 butir bukti, 0 kontributor (jujur). Gate hijau (tsc/lint/check:editorial/
  check:lab-isolation/build, `/misi` + `/api/lab/promote` ter-build).
- **Mendekati Checkpoint 3:** Bucket C tinggal 3.5 (Ghost Signals, v1.1). Loop penuh
  kini hidup: harvest -> skor -> dashboard -> **promosi jadi pertanyaan publik** ->
  /misi -> (lapor warga via ReportForm yg sudah ada, mission_id bisa menaut).
- **Catatan:** ada tabel `alert_events`/`watch_alerts` (RLS-no-policy) di DB dari
  kerja paralel (kemungkinan Ghost Signals 3.5), bukan dari sesi ini.

### 29 Juni 2026 (lanjutan), Bucket C Step 3.3: Lazarus Score + dashboard /lab + MODE OTONOMI PENUH (founder-directed)

Founder mengaktifkan **MODE OTONOMI PENUH**: dilarang minta founder ambil token/
set env/jalankan skrip; wajib bangun fallback/mock/sample agar UI+logika bisa
kuverifikasi sendiri di browser. Lihat [[full-autonomy-fallbacks]]. Push `fcb3f35`.

- **Skor Lazarus** (`lib/lab/scoring.ts`, pure+deterministik): 0-100 dari signals
  harvester. Bobot: jeda waktu 0,45 + IUCN 0,25 + kelangkaan 0,20 (penambah),
  observasi iNat riset terbaru 0,30 (PENGURANG, karena spesies yang masih terlihat
  bukan prioritas pencarian). Output `breakdown[]` transparan (tiap poin bisa
  dijelaskan). **Heuristik prioritas, BUKAN probabilitas keberadaan.**
- **Fallback IUCN kurasi** (`scripts/lab/seed-iucn.mjs`): kategori IUCN publik
  NYATA untuk 12 taksa, provenance ditandai `curated` (vs `api`), plus verify-link.
  Memberi data skor lengkap tanpa token; API live menimpa saat token ada. Tidak
  mengarang fakta. `harvest-iucn.mjs` kini pakai ini saat token kosong (bukan no-op).
- **Snapshot data nyata** (`lib/lab/sample-leads.ts` via `build-leads --emit-sample`):
  GBIF+iNat live + IUCN kurasi. `getScoredLeads()` fallback DB->sample (dilabel
  **CONTOH** di UI), jadi dashboard selalu terisi di dev & prod, tak pernah kosong.
- **Dashboard** (`app/lab/page.tsx` + client `components/lab/LabLeadsBoard.tsx`):
  urut/filter/cari + kartu lead yang bisa diklik buka **rincian skor** (poin per
  sinyal + bar + catatan bersumber + tautan langsung GBIF/iNat/IUCN). Tombol
  **"Promosikan ke Misi" disabled** = seam Fase 3 (disambung di 3.4). Dua banner:
  internal "belum terverifikasi" + provenance CONTOH.
- **Dev bypass lokal** (`LAB_DEV_BYPASS` di `middleware.ts` + `lib/lab/auth.ts`):
  **mustahil di produksi** (dijaga `NODE_ENV !== "production"` + flag hanya di
  `.env.local` gitignored), agar /lab yang admin-gated bisa kuscreenshot tanpa sesi
  admin nyata. Tak pernah ikut ke deploy.
- **Verifikasi browser (Playwright, dev, bypass):** skor membedakan dengan benar,
  taksa benar-benar hilang naik (Zaglossus attenboroughi 93, Cyornis ruckii 92,
  Varanus zugorum 85, Gallirallus sharpei 72), spesies dgn banyak rekaman terbaru
  turun (harimau Jawa captive 6, badak 9, seriwang 11). Desktop + mobile 360px
  bersih, rincian + tautan sumber render. Gate hijau (tsc/lint/check:editorial/
  check:lab-isolation/build exit 0, `/lab` ƒ dynamic).
- **Berikutnya:** Step 3.4 misi->DB + promosi 1-klik lead->"Misi Verifikasi
  Lapangan" (menyambung tombol disabled + seam /admin/reports). Lalu 3.5 Ghost
  Signals. **Blocker run nyata kini OPSIONAL** (ada fallback): service-role key
  (tulis DB) + IUCN token (IUCN live). Lihat `docs/lab-architecture.md`.

### 29 Juni 2026 (lanjutan 4), Peluncuran NaLI Intelligence Platform v1.0 (founder-directed)

Meta-task penutup epik: **pakai platform baru untuk mengumumkan platform baru.**
Mode otonomi penuh + auto-push. Push `3f59b15`.

- **Artikel peluncuran terbit** (`content/articles/nali-intelligence-platform-jurnal-yang-hidup.mdx`,
  kategori catatan-lapangan, confidence high, ~915 kata): "Memperkenalkan NaLI
  Intelligence Platform: Jurnal yang Hidup dan Jujur". Menjelaskan 4 fitur pembaca
  (Knowledge Genome, Tutor Tanya NaLI, Jalur Baca, Misi Sains Warga) + model isolasi
  Lab -> gerbang manusia -> misi. **Artikel ini SEKALIGUS demo Knowledge Genome**:
  memakai frontmatter penuh (claimLedger 5 baris semua "terverifikasi kuat",
  confidence, evidenceBasis "sumber terbuka", articleStatus terkini) supaya kartu
  gizi epistemik tampil di tulisan pengumuman itu sendiri.
- **4 source entry baru** yang jujur menopang klaim: halaman /metodologi + /pedoman-sumber
  NaLI (dokumen primer komitmen sendiri, ditandai BUKAN bukti faktual luar), GBIF
  occurrence portal, iNaturalist observations (tulang punggung data terbuka Lab).
- **Diagram SVG buatan NaLI** alur Lab -> gerbang -> misi (6 langkah, navy archive-ink,
  CC BY 4.0), di-inline di body via `![](src)` (ingat gotcha: tanpa inline tak render).
- **README ditulis ulang**: NaLI kini "Intelligence Platform", mendokumentasikan fitur
  pembaca, Lab internal, dan **model isolasi satu arah dengan diagram alur ASCII**.
- **Homepage tidak di-hardcode**: "Tulisan terbaru" force-dynamic, jadi artikel
  peluncuran (tanggal hari ini) otomatis jadi yang teratas. Sesuai prinsip
  "build once, auto-update", tidak menanam featured-article manual.
- **Gate hijau**: tsc 0, lint clean (hanya warn ShareButton img pre-existing),
  check:editorial PASS (word floor 900 dilewati; warn method/meta ratio 40% wajar
  untuk artikel meta), check:lab-isolation OK, build exit 0. Push `c35f6c9..3f59b15`.
- **PLATFORM v1.0 RESMI DILUNCURKAN.** Bucket A + B + C lengkap, semua live di
  nalibynative.com, kontrak epistemik utuh di tiap lapisan.

### 29 Juni 2026 (lanjutan 5), Perombakan homepage + navigasi (user-friendliness) + audit + resilience (founder-directed)

Founder pusing/bingung landing di NaLI: fitur tersembunyi, homepage "tak berubah",
navigasi sulit. Dua push.

- **Audit + resilience (push `aa4f501`)**: founder kirim audit (AI-generated) klaim
  "/misi rusak total" + banyak gap SEO. Investigasi: **/misi SEHAT** (prod 200 + SSR
  konten penuh; tabel `missions` kosong -> fallback 2 seed valid; hydrate lokal 0 error
  console). Crash = transient/artefak. Mayoritas alarm audit FALSE POSITIVE (sitemap
  2.817 URL bukan kosong; JSON-LD/OG/Twitter/canonical ADA; CSP+header keamanan lengkap
  ADA; Cmd+K search ADA). Cacat NYATA yang diperbaiki: (1) **TIDAK ada error boundary** =
  sebab layar putih "client-side exception" -> tambah `app/error.tsx` + `app/global-error.tsx`
  (inline-styled, navy `#0E3A5C` hardcoded sengaja agar tahan stylesheet gagal); (2)
  `og:image` SVG untuk artikel diagram-only (sosial tak render SVG) -> skip .svg, fallback
  ke raster `/api/og`; (3) `normalizeMission()` di `lib/missions.ts` (coerce DB row+seed ke
  shape aman). Lihat [[site-audit-baseline]].
- **Perombakan homepage + nav (push `da45ad5`)**: ganti landing NeoMuseum animatif (yang
  SEMBUNYIKAN nav + kubur fitur) jadi "front door" jelas. `app/page.tsx`: hero (judul + 3
  CTA) -> Fitur Unggulan (Tanya NaLI / Knowledge Genome / Jalur Baca / Sains Warga) ->
  artikel peluncuran (badge BARU) -> misi aktif + progress -> preview Jalur Baca -> tulisan
  terbaru + newsletter (semua data nyata). `components/Nav.tsx`: Beranda / Artikel
  (dropdown: Semua/Alam/Sejarah/Investigasi) / Jelajah [Interactive] / Misi Warga
  [Citizen Science] / Tentang + search + theme; drawer mobile dapat search bar menonjol +
  accordion Artikel + grid "Lainnya" (Jurnal/Pustaka/Arsip/Seri/Ruang Kendali/Kontak tetap
  terjangkau). `/lab` & `/admin` tak pernah tampil. `SiteChrome`: homepage kini pakai
  Nav+Footer bersama (tak lagi opt-out). Artikel: banner "Tahukah Anda?" di bawah Knowledge
  Genome (tunjuk AI Tutor + Jalur Baca). NeoMuseum/DailyBand/KnowledgeShelves/ContinueRail
  kini dead code di home (recoverable). Verifikasi browser 1440px + 360px (drawer + accordion
  + badge, 0 error console). Gate hijau (tsc/lint/check:editorial/check:lab-isolation/build).

### 30 Juni 2026, Lab dinyalakan sungguhan: data real ke prod + harvest-log + cron mingguan (founder-directed)

Founder meneruskan prompt AI-generated yang mengklaim sistem Lazarus "BELUM PERNAH
JALAN" dan "mesin mati". Diperlakukan seperti audit forwarded ([[site-audit-baseline]])
dan ABYSS-WALKER: cek klaim dulu, jangan telan/jangan tolak buta.
- **Reality-check:** klaim "tak pernah jalan" SALAH , harvester jalan (kubuktikan
  dua kali live: `lab:harvest:gbif` + `:inat` ke API GBIF/iNaturalist, read-only,
  tanpa kunci; data hari ini). Sample selama ini memang data harvest nyata berlabel
  CONTOH, bukan fabrikasi. Hiperbola "Palantir" diabaikan. **Satu poin valid:** tabel
  `lab_leads` prod memang KOSONG -> `/lab` menyajikan fallback sample, bukan baris DB.
- **Ditolak dari prompt:** commit raw dump (content/lab-raw sengaja gitignored, data
  pihak ketiga regenerable), email Resend/SendGrid (dep berbayar + kredensial;
  halaman harvest-log SUDAH jadi surface laporan).
- **Yang dikerjakan:** distilasi 12 lead dari harvest hari ini (logika `build-leads`
  yang sudah teruji) lalu **ditulis ke `lab_leads` prod via Supabase MCP** (tanpa
  kredensial founder). `/lab` kini `source:"db"` (lead nyata, bukan CONTOH). Cerita
  jujur langsung tampak: Zaglossus attenboroughi jeda 65thn, Cyornis ruckii/Varanus
  zugorum/Gallirallus sharpei nyaris/0 rekaman vs pesut/badak/Nepenthes terlihat 2022-2026.
- **Jejak audit (jawab "mana buktinya"):** migrasi `0013` `public.lab_harvest_runs`
  (RLS admin-only, cermin lab_leads; advisor bersih) + `lib/lab/harvest-log.ts` +
  `app/lab/harvest-log/page.tsx` (server component) + `components/lab/HarvestLog.tsx`,
  ditaut dari header `/lab`. TANPA fallback sample (memalsukan jejak audit = tidak
  jujur; empty-state di dev/anon benar, baris asli hanya tampil ke sesi admin via RLS).
  Run pertama tercatat (id 1, 30 Jun). `build-leads.mjs` kini log 1 baris per run
  (`--trigger manual|cron|dev|backfill`, tak pernah gagalkan harvest).
- **Otomasi:** `.github/workflows/lab-harvest.yml` , mingguan Senin 02:00 UTC (09:00
  WIB) + dispatch manual, re-harvest + `build-leads --trigger cron`. INERT sampai
  founder tambah repo secret `SUPABASE_SERVICE_ROLE_KEY` (guard step no-op tanpa itu;
  IUCN_API_TOKEN opsional). Tidak commit dump; jejak = lab_harvest_runs + riwayat
  Actions. **Satu langkah founder:** tambah secret di Settings -> Secrets -> Actions.
- **Verified:** tsc 0, lint clean (hanya warn ShareButton lama), check:editorial PASS
  (em-dash bersih, pakai koma), check:lab-isolation OK, build exit 0 (`/lab/harvest-log`
  ter-build). Auto-push per [[auto-push-after-build]]. Lihat [[reader-intelligence-roadmap]].

### 30 Juni 2026 (lanjutan), Lab SELF-RUNNING tanpa kredensial founder: edge function + pg_cron (founder: "kamu yang lakukan dan selesaikan itu semua")

Founder menolak langkah manual GitHub secret. Jujur: aku TIDAK bisa set secret
service-role di GitHub (tak punya nilainya + dilarang menangani kunci rahasia).
Tapi ada jalur Supabase-native yang TIDAK butuh kredensial founder sama sekali.
- **Edge Function `lab-harvest`** (`supabase/functions/lab-harvest/index.ts`,
  deploy via MCP, verify_jwt=true): port logika harvest+distill `build-leads` ke
  Deno; embed 12 taksa dengan GBIF usageKey + iNat taxon_id terverifikasi (lewati
  langkah match nama yang rapuh). Panen GBIF+iNat live (read-only), upsert
  `lab_leads` + log `lab_harvest_runs` pakai **SUPABASE_SERVICE_ROLE_KEY yang
  di-inject Supabase otomatis ke runtime fungsi** , kunci tak pernah keluar dari
  lingkungan Supabase, aku tak pernah menanganinya. Upsert tak timpa status/notes.
- **Jadwal mingguan** (`supabase/migrations/0014`): `pg_cron` + `pg_net`,
  Senin 02:00 UTC (09:00 WIB), `net.http_post` ke fungsi dengan **anon key PUBLIK**
  (publishable, sama dengan yang di frontend, aman disimpan di cron/migration).
  Job `lab-harvest-weekly` aktif. INI penjadwal utama; `.github/workflows/lab-harvest.yml`
  diturunkan jadi **manual-dispatch fallback** saja (tak lagi punya `schedule:`).
- **Diverifikasi end-to-end (bukan cuma terdaftar):** (1) invoke fungsi via curl
  anon -> `{"status":"success","taxa":12,"upserted":12}` (run #2). (2) jalankan
  PERSIS perintah `net.http_post` yang dipakai cron -> run #3 `trigger=cron`
  `status=success` 12 lead. Jadi rantai `pg_cron -> pg_net -> edge fn -> DB`
  terbukti hidup. `tsconfig` exclude `supabase/functions` (Deno/esm.sh imports tak
  boleh kena tsc Next). Isolation aman (fungsi standalone, bukan import kode lab
  publik). Gate hijau (tsc 0/lint/check:editorial/check:lab-isolation/build).
- **Hasil:** mesin Lazarus kini benar-benar jalan sendiri tiap minggu, MELAPOR ke
  `/lab/harvest-log`, **tanpa satu pun langkah/kredensial founder**. GH Actions
  (dengan secret) tetap ada sebagai pemicu manual opsional.

### 30 Juni 2026 (lanjutan 2), Karakter operator Lab real-time + verdict sumber data (forwarded prompt)

Founder meneruskan prompt AI 2 misi: (1) tambah ~31 sumber data, (2) karakter
"Minecraft" real-time di /lab. Diperlakukan seperti forwarded prompt lain: cek
dulu, kerjakan yang baik, tolak yang merugikan.
- **MISI 2 (karakter) DIKERJAKAN PENUH + diverifikasi:** `lib/lab/status.ts`
  (baca `lab_harvest_runs`, derive state working/idle/sleeping dari timestamp run
  terakhir: <=30min kerja, <=24j istirahat, >24j tidur; sumber aktif dari providers
  run terakhir; next run = Senin 02:00 UTC; sukses/total). `app/api/lab/status`
  (same-origin + admin, mirror promote, cache 10s). `components/lab/LabCharacter.tsx`
  (client, poll 30s, SVG pixel-art ORISINAL bukan aset Minecraft, 3 state: ketik di
  meja / kopi berasap / tidur dengan zzz; lampu status hijau/amber/merah; klik =
  dialog acak; `?karakter=working|idle|sleeping` override visual untuk
  screenshot/preview, tak ubah data; reduced-motion aman). Dipasang di `/lab` atas.
  **Verified via Claude Preview (dev-bypass):** 3 state render benar (label MESIN
  SEDANG BEKERJA/ISTIRAHAT/TIDUR), 0 error konsol. Di dev anon, RLS blokir
  lab_harvest_runs -> tampil 0 sumber (benar; data nyata hanya ke sesi admin prod).
- **MISI 1 (sumber data) DI-CURATE, BUKAN 31 borongan:** ditolak Reddit/X/Google
  News sebagai SINYAL skor (chatter != bukti, langgar anti-spekulasi), Google
  Scholar scraping (langgar ToS), satelit/LiDAR/genetik (butuh auth/skala besar,
  fitur lain bukan lead-scoring). Disetujui untuk ditambah BERTAHAP (satu harvester
  hati-hati per pass, krn tiap sumber ubah scoring): Wikipedia/Wikidata (+pageviews
  sbg konteks), foto Wikimedia/GBIF/iNat, OBIS (laut), eBird. Belum dikerjakan
  sesi ini (karakter diprioritaskan + dikerjakan tuntas).
- **impeccable hook**: warna hex pixel-art ditandai (di luar token archive-ink) ->
  sengaja (palet ilustrasi voxel mandiri, seperti GalaxyGraph/ShadowGraph), dibiarkan,
  TANPA persist ignore (perlu konfirmasi founder).
- **Verified:** tsc 0, lint clean, check:editorial PASS (karakter em-dash placeholder
  diganti hyphen), check:lab-isolation OK (status route di app/api/lab = lab path),
  build exit 0 (`/api/lab/status` ƒ). Auto-push per [[auto-push-after-build]].

### 30 Juni 2026 (lanjutan 3), Karakter Lab di-upgrade ke 3D ruang gaya The Sims (founder-directed)

Founder menolak versi 2D SVG statis: minta **3D, kamera gaya The Sims yang menyorot
karakter di dalam kamar lengkap** (meja, komputer, keyboard, mouse, monitor), dengan
aktivitas penuh di kamar secara real-time: kerja di depan komputer kalau bekerja,
jalan ke kursi lalu duduk menyeruput kopi + merokok kalau istirahat, jalan ke kasur
lalu tidur kalau offline.
- **`components/lab/LabRoom3D.tsx` (BARU):** scene three.js prosedural mandiri (TANPA
  dep baru, `three@0.184` + `@types/three` sudah ada dari GalaxyGraph; TANPA model
  GLTF eksternal/aset Minecraft). Satu kamar: lantai+karpet, 2 dinding, poster
  "spesies hilang", meja kerja + monitor (bar data menyala) + keyboard + mouse + tower
  LED + kursi kantor, kursi santai (oxblood) + meja samping, kasur (bantal + selimut
  trim teal). Karakter voxel humanoid prosedural (kepala/rambut/mata/torso/lengan ber-
  siku/kaki ber-lutut, semua BoxGeometry). Kamera perspektif sudut 3/4 tinggi (Sims)
  dengan sway halus "hidup".
- **State machine real-time:** prop `state` (working/idle/sleeping) -> 3 STATION
  (meja/lounge/kasur). Saat state berubah, karakter **BERJALAN** (lerp posisi + ayun
  kaki) lalu ease ke pose: working=duduk mengetik (lengan menunduk ke keyboard, bar
  monitor menari, point-light monitor berdenyut); idle=duduk di lounge, mug kopi +
  rokok+bara + asap/uap mengepul, gerakan menyeruput periodik; sleeping=berbaring di
  kasur (kepala di bantal, selimut, napas pelan, sprite "Z" naik dari kepala). Warna
  lampu/monitor ikut state (teal/amber/merah).
- **`LabCharacter.tsx` dirombak:** SVG `Scene/DeskScene/SleepScene` + keyframe CSS lama
  DIHAPUS, diganti `<LabRoom3D state={state} />` sebagai banner 280px (chip status +
  bubble dialog overlay tetap; readout lead/run/panen + polling 30s + `?karakter=`
  override + klik-untuk-menyapa tetap). Data path & `/api/lab/status` TIDAK berubah.
- **Perf & a11y:** DPR cap 2, pause via `visibilitychange` + IntersectionObserver
  (berhenti rAF saat tab tersembunyi / offscreen), dispose penuh geometry/material/
  renderer saat unmount, ResizeObserver. prefers-reduced-motion -> render SATU frame
  statis (karakter sudah di station, tanpa rAF).
- **Gotcha lying-pose:** rotasi badan -90deg di pivot kaki menjatuhkan karakter ke
  LANTAI di samping kasur (kelihatan saat verifikasi). Diperbaiki: rotasi +90deg (kepala
  ke +z), angkat badan ke tinggi kasur (`BED_Y=0.95`) + geser `z=-1.12*lying` agar
  terpusat, station sleeping `face=0` selaras sumbu kasur (kasur rotate PI/2 -> panjang
  di world-z). Setelah fix: karakter jelas berbaring DI kasur, kepala di bantal.
- **Verified via Claude Preview (dev-bypass, port 3100):** ketiga state ter-screenshot
  benar (working di meja teal / idle di lounge amber / sleeping di kasur merah + Z),
  0 error konsol, WebGL render bersih. Walk = real-time saat prop `state` berubah pada
  poll 30s (deterministik lerp). **Gotcha build dikonfirmasi ulang:** build saat dev
  server `next dev` jalan -> `.next` clobber -> `PageNotFoundError /admin/contradictions`
  palsu; solusi: stop dev, `rm -rf .next`, build bersih.
- Gate hijau (tsc 0/lint/check:editorial/check:lab-isolation/build). Auto-push per
  [[auto-push-after-build]].

### 30 Juni 2026 (lanjutan 4), Lab room 3D diperbaiki + sistem siang/malam real-time (founder-directed)

Founder menolak versi 3D pertama (banyak cacat + "kamar hantu/kota mati"). Diperbaiki
menyeluruh, tiap cacat diverifikasi per screenshot (founder: "kalau nemu cacat di
screenshot jangan oke oke saja").
- **Sistem cahaya siang/malam real-time** (`sunForHour(hour)` di LabRoom3D): matahari
  bergerak melintasi JENDELA (busur timur->barat, ketinggian ikut jam), warna hangat
  saat fajar/senja -> netral siang; ambient hemisphere + background + kabut ikut jam;
  **god-ray shaft** + debu melayang dari jendela menguat saat siang; malam = kamar gelap
  bertenaga lampu meja hangat + cahaya monitor + bulan sabit + bintang, dengan **bloom**
  (UnrealBloomPass dari three/examples) untuk kilau realistis. (BUKAN path tracing
  sungguhan, dijelaskan jujur ke founder: itu butuh GPU berat, tak cocok widget kecil.)
- **State activity = jam + kesehatan mesin** (`deriveState` di LabCharacter): malam
  (20:00-06:00) -> tidur; siang baru selesai panen (<=90 menit) -> rehat ngopi; siang
  lainnya -> bekerja. Jadi operator "bekerja terus" sepanjang siang tiap hari, bukan
  tidur 6 hari karena cron mingguan. Lampu status + readout tetap lapor kesehatan mesin
  nyata (panen terakhir/berikutnya/lead) secara jujur.
- **Cacat 3D yang diperbaiki (semua diverifikasi browser):** (1) kursi + karakter kerja
  kini MENGHADAP monitor (meja pindah ke dinding kiri, monitor hadap +x, karakter hadap
  -x; tangan mengetik + klik mouse). (2) Lounge dipindah ke depan-tengah, MENGHADAP
  kamera (wajah terlihat), tak lagi nabrak kasur. (3) Kasur: headboard dipindah ke ujung
  JAUH (-z) + direndahkan supaya kepala+wajah si tidur (ujung dekat kamera) tak
  terhalang; selimut diberi warna kontras (#8a9bb0) karena tadinya sewarna baju navy jadi
  badan menyatu/hilang; `BED_Y` dinaikkan agar kepala di atas bantal. (4) Lampu lantai
  yang SILAU dikecilkan drastis (emissive + intensitas + bloom turun). (5) Framing kamera
  diperlebar (FOV 45, kamera mundur, lookAt digeser) agar tiga zona (meja/lounge/kasur)
  semua masuk.
- **Bug hidrasi DIPERBAIKI:** `hour` dari `new Date()` beda antara SSR & klien -> "Text
  content did not match". Solusi: gate `mounted` (nilai jam deterministik 12 saat SSR,
  jam asli setelah mount), pola sama seperti ThemeToggle.
- **Override screenshot:** `?karakter=working|idle|sleeping` + `?jam=0..23` (visual saja,
  tak ubah data). Verified 5 kondisi (kerja siang, rehat siang, tidur dinihari-terang,
  malam tidur, framing) via Claude Preview dev-bypass, 0 error konsol.
- Gate hijau (tsc 0/lint/check:editorial/check:lab-isolation/build). Auto-push per
  [[auto-push-after-build]].

---

*Last updated: 30 Juni 2026 (lanjutan 3), **Karakter Lab di-upgrade ke 3D ruang gaya The Sims**. Founder menolak versi 2D SVG statis, minta 3D kamera Sims menyorot karakter di kamar lengkap (meja/PC/keyboard/mouse/monitor) dengan aktivitas real-time: kerja di depan PC kalau bekerja, jalan ke kursi lalu duduk ngopi+merokok kalau istirahat, jalan ke kasur lalu tidur kalau offline. `components/lab/LabRoom3D.tsx` BARU: scene three.js prosedural mandiri (TANPA dep baru, `three@0.184`+`@types/three` sudah ada; TANPA model GLTF/aset Minecraft), satu kamar (lantai+karpet+2 dinding+poster, meja+monitor bar-data+keyboard+mouse+tower+kursi kantor, kursi santai oxblood+meja samping, kasur bantal+selimut trim teal), karakter voxel humanoid (siku/lutut ber-pivot dari BoxGeometry), kamera 3/4 tinggi + sway hidup. State machine: prop `state` -> 3 STATION; saat berubah karakter BERJALAN (lerp posisi+ayun kaki) lalu ease ke pose (working=duduk ketik+bar monitor menari+point-light denyut; idle=duduk lounge+mug kopi+rokok+bara+asap/uap+gerak seruput; sleeping=berbaring di kasur kepala di bantal+napas+sprite Z). Warna lampu/monitor ikut state (teal/amber/merah). `LabCharacter.tsx` dirombak: SVG Scene/DeskScene/SleepScene+keyframe CSS DIHAPUS -> `<LabRoom3D/>` banner 280px (chip status+bubble+readout+polling 30s+`?karakter=` override+klik-menyapa tetap; data path & /api/lab/status TAK berubah). Perf/a11y: DPR cap 2, pause visibilitychange+IntersectionObserver, dispose penuh, ResizeObserver, reduced-motion=1 frame statis. Gotcha lying-pose: rotasi badan -90deg pivot kaki menjatuhkan karakter ke LANTAI samping kasur -> fix +90deg (kepala +z)+angkat BED_Y=0.95+geser z=-1.12*lying+station face=0 selaras sumbu kasur. Verified via Claude Preview dev-bypass (port 3100): 3 state ter-screenshot benar (working meja teal/idle lounge amber/sleeping kasur merah+Z), 0 error konsol, WebGL bersih; walk real-time saat prop berubah pada poll 30s. Gotcha build dikonfirmasi: build saat `next dev` jalan -> .next clobber -> PageNotFoundError /admin/contradictions palsu; solusi stop dev, `rm -rf .next`, build bersih. Gate hijau (tsc 0/lint/check:editorial/check:lab-isolation/build). Auto-push per [[auto-push-after-build]]. Sebelumnya: 30 Juni 2026 (lanjutan 2), **Karakter operator Lab real-time (2D) + verdict sumber data**. Forwarded prompt AI 2 misi. MISI 2 (karakter "Minecraft" real-time) DIKERJAKAN PENUH: `lib/lab/status.ts` (state working/idle/sleeping dari timestamp lab_harvest_runs: <=30min/<=24j/>24j) + `app/api/lab/status` (admin+same-origin, cache 10s) + `components/lab/LabCharacter.tsx` (client poll 30s, SVG pixel-art ORISINAL bukan aset Minecraft, 3 state ketik/kopi-berasap/tidur-zzz, lampu hijau/amber/merah, klik=dialog, `?karakter=` override visual, reduced-motion aman) dipasang di /lab. Verified via Claude Preview dev-bypass: 3 state render benar + 0 error konsol (di dev RLS blokir data->0 sumber, benar; data nyata hanya ke admin prod). MISI 1 (~31 sumber) DI-CURATE bukan borongan: TOLAK Reddit/X/Google News sbg sinyal (chatter!=bukti), Google Scholar scrape (ToS), satelit/LiDAR/genetik (auth/skala, fitur lain); SETUJU bertahap Wikipedia/Wikidata+pageviews, foto Wikimedia/GBIF/iNat, OBIS, eBird (belum dikerjakan, karakter diprioritaskan). impeccable hex pixel-art = sengaja (palet voxel mandiri). Gate hijau (tsc 0/lint/check:editorial/check:lab-isolation/build exit 0). Sebelumnya: 30 Juni 2026 (lanjutan), **Lab SELF-RUNNING tanpa kredensial founder** (founder: "kamu yang lakukan dan selesaikan itu semua"). Tak bisa set GitHub secret (tak punya nilai + dilarang tangani kunci), jadi pakai jalur Supabase-native: Edge Function `lab-harvest` (`supabase/functions/lab-harvest/index.ts`, deploy via MCP, port harvest+distill ke Deno, embed 12 taksa GBIF/iNat key terverifikasi, tulis lab_leads+lab_harvest_runs pakai SERVICE_ROLE yang DI-INJECT Supabase ke runtime fungsi , kunci tak pernah kutangani) + jadwal `pg_cron`+`pg_net` mingguan Senin 09:00 WIB (migrasi `0014`, auth pakai anon key PUBLIK). Job `lab-harvest-weekly` aktif. GH Actions diturunkan jadi manual-dispatch fallback. Diverifikasi end-to-end: invoke fungsi -> success 12 lead (run #2); jalankan persis perintah net.http_post cron -> run #3 trigger=cron success (rantai pg_cron->pg_net->edge->DB terbukti). tsconfig exclude supabase/functions. Mesin Lazarus kini jalan sendiri tiap minggu + lapor ke /lab/harvest-log, TANPA langkah founder. Sebelumnya hari ini: **Lab dinyalakan sungguhan** (harvest live -> 12 lead nyata ditulis ke `lab_leads` prod via Supabase MCP, `/lab` kini source:"db" bukan CONTOH; migrasi `0013` `lab_harvest_runs` + `/lab/harvest-log` audit page + `components/lab/HarvestLog.tsx` + `lib/lab/harvest-log.ts`, ditaut dari /lab; `build-leads.mjs` log 1 baris/run via `--trigger`; `.github/workflows/lab-harvest.yml`). Dipicu prompt forwarded "sistem tak pernah jalan" -> reality-check: SALAH (harvester jalan, kubuktikan 2x live; sample = data nyata berlabel CONTOH), satu poin valid (lab_leads kosong, kini diisi). Ditolak: commit raw dump (gitignored sengaja), email berbayar. Gate hijau (tsc 0/lint/check:editorial PASS/check:lab-isolation/build exit 0). Lihat [[reader-intelligence-roadmap]]. Sebelumnya: 29 Juni 2026 (lanjutan 5), **perombakan homepage + navigasi** (push `da45ad5`) + audit/resilience (push `aa4f501`). Founder bingung landing: fitur tersembunyi, nav sulit. Audit forwarded (AI) klaim "/misi rusak" -> investigasi: /misi SEHAT (200+SSR penuh, missions table kosong->2 seed valid, hydrate 0 error); mayoritas alarm SEO FALSE POSITIVE (sitemap 2817 URL, JSON-LD/OG/Twitter/canonical/CSP/search semua ADA). Cacat nyata diperbaiki: error boundary `app/error.tsx`+`app/global-error.tsx` (sebab "client-side exception" full-page = TANPA error boundary, bukan bukti error klien), og:image skip SVG->raster /api/og, normalizeMission() guard. Lihat [[site-audit-baseline]]. Lalu homepage dirombak total: NeoMuseum animatif (sembunyikan nav) -> "front door" jelas (hero+3 CTA, Fitur Unggulan 4 kartu, artikel peluncuran BARU, misi aktif, preview Jalur Baca, tulisan terbaru+newsletter). Nav disederhanakan: Beranda/Artikel(dropdown Semua-Alam-Sejarah-Investigasi)/Jelajah[Interactive]/Misi Warga[Citizen Science]/Tentang; drawer mobile dapat search bar+accordion+grid Lainnya; /lab & /admin tak tampil. SiteChrome: homepage pakai Nav+Footer bersama. Artikel: banner "Tahukah Anda?" (AI Tutor+Jalur Baca) di bawah Knowledge Genome. NeoMuseum/DailyBand/KnowledgeShelves/ContinueRail dead code di home (recoverable). Verifikasi 1440px+360px (0 error). Gate hijau. Sebelumnya: 29 Juni 2026 (lanjutan 4), **PELUNCURAN NaLI Intelligence Platform v1.0** (push `3f59b15`). Meta-task: pakai platform untuk mengumumkan platform. Artikel peluncuran `content/articles/nali-intelligence-platform-jurnal-yang-hidup.mdx` (kategori catatan-lapangan, confidence high, ~915 kata) menjelaskan 4 fitur pembaca (Knowledge Genome, Tutor Tanya NaLI, Jalur Baca, Misi Sains Warga) + model isolasi Lab->gerbang manusia->misi, dan SEKALIGUS jadi demo Knowledge Genome (frontmatter penuh: claimLedger 5 baris "terverifikasi kuat", evidenceBasis "sumber terbuka", articleStatus terkini). 4 source baru (metodologi+pedoman-sumber NaLI sbg dokumen primer komitmen sendiri BUKAN bukti faktual luar, GBIF occurrence portal, iNaturalist observations). Diagram SVG buatan NaLI alur Lab->gerbang->misi (CC BY 4.0) di-inline di body. README ditulis ulang jadi "Intelligence Platform" + diagram alur ASCII model isolasi satu arah. Homepage tak di-hardcode (force-dynamic auto-feature artikel terbaru). Gate hijau (tsc/lint/check:editorial PASS/check:lab-isolation/build exit 0). Total artikel terbit = 50. Bucket A+B+C lengkap, platform v1.0 live di nalibynative.com. Sebelumnya: 29 Juni 2026 (lanjutan 3), Bucket C Step 3.5 Ghost Signals + **CHECKPOINT 3 (Bucket C) SELESAI** (push `c6ea410`). Rekonsiliasi: `watch_alerts`/`alert_events` ternyata fitur LAIN (notifikasi perubahan konten internal), dibiarkan utuh; dibuat tabel khusus `ghost_signals` (0012 via MCP: source youtube/xeno-canto/inaturalist, external_id, title, url, observed_on, location_label, taxon_hint, summary, score, signals, provenance api/curated/sample, status; RLS admin-only tanpa anon, unique(source,external_id), advisor bersih). Monitor `scripts/lab/ghost/`: harvest-inat-needsid (NYATA tanpa key, filter belum-teridentifikasi-ke-spesies), harvest-xenocanto (v2 mati->v3 butuh XENOCANTO_API_KEY, fallback sample berlabel), harvest-youtube (butuh YOUTUBE_API_KEY, fallback sample), build-signals (skor kemenarikan transparan misteri 0,45+kebaruan 0,35+dapat-dilacak 0,20; --emit-sample). Snapshot `lib/lab/sample-signals.ts` (iNat nyata + CONTOH). `/lab/signals`+`GhostSignalsBoard`+`lib/lab/ghost.ts` (DB->sample). `/api/lab/promote` kini terima leadId ATAU signalId -> misi "Verifikasi lapangan: X" (pertanyaan, "bukan klaim"), flip status promoted. Verifikasi browser via Claude Preview: /lab/signals tampil (Katydids 86, Softshell Turtles 82) + expand rincian + promote -> /misi misi badge "Berasal dari investigasi Lab"+"bukan klaim". **ARSITEKTUR LAB LENGKAP:** harvest (read-only->content/lab-raw gitignored) -> skor transparan (Lazarus+ghost, HEURISTIK bukan probabilitas) -> dashboard privat admin-gated (/lab + /lab/signals, fallback DB->sample CONTOH) -> gerbang manusia (promosi 1-klik) -> SATU artefak publik = MISI pertanyaan (bukan klaim) -> /misi -> lapor warga (ReportForm mission_id). Tiga lapis isolasi: RLS admin-only, check:lab-isolation, kontrak promosi satu arah. Semua key run-nyata OPSIONAL (ada fallback). Gate hijau (tsc/lint/check:editorial/check:lab-isolation/build). Sebelumnya: Step 3.4 promosi lead -> Misi (push `71446b6`). Tombol "Promosikan ke Misi" (disabled di 3.3) kini hand-off Lab -> publik nyata. Migrasi `missions` (0011 via MCP): `public.missions` (id slug, status active/closed, source editorial/lab, lead_id FK->lab_leads, evidence_needed, progress; RLS publik baca + admin tulis); advisor bersih. `lib/missions.ts` async: DB -> seed JSON editorial -> (non-prod) `content/missions/_lab-dev` gitignored, dedup per id (DB menang), JSON jamin /misi tak kosong. `types/missions.ts`+source/leadId; `/misi` async + badge "Berasal dari investigasi Lab" (source=lab) + anchor #misi-<id>. `app/api/lab/promote` (di app/api/lab=lab path): same-origin+admin/dev-bypass, ambil lead via getScoredLeads, **generate PERTANYAAN bukan klaim** ("Cari bukti terkini... ini bukan klaim... pertanyaan terbuka") +4 butir bukti; prod upsert missions idempotent + lead->promoted, dev-bypass tulis JSON _lab-dev (fs) agar verifiable tanpa service key. Isolation: lib/missions TIDAK impor lab. `LabLeadsBoard` tombol promote aktif -> POST -> state "Misi dibuat"+link. Verifikasi browser via Claude Preview (Playwright disconnect): promote Zaglossus 93 -> sukses -> /misi misi baru badge+judul pertanyaan+"bukan klaim"+4 bukti+0 kontributor jujur. Loop penuh hidup: harvest->skor->dashboard->promosi pertanyaan publik->/misi. Tabel alert_events/watch_alerts (RLS-no-policy) dari kerja paralel (mungkin Ghost Signals 3.5), bukan sesi ini. Bucket C sisa hanya 3.5. Gate hijau (tsc/lint/check:editorial/check:lab-isolation/build). Sebelumnya: Bucket C Step 3.3 Lazarus Score + dashboard /lab (push `fcb3f35`) + MODE OTONOMI PENUH (founder: jangan minta token/env/skrip; bangun fallback agar self-verifiable di browser, [[full-autonomy-fallbacks]]). `lib/lab/scoring.ts` skor 0-100 deterministik (jeda 0,45 + IUCN 0,25 + kelangkaan 0,20 penambah, observasi iNat terbaru 0,30 pengurang), `breakdown[]` transparan, HEURISTIK bukan probabilitas. Fallback IUCN kurasi (`scripts/lab/seed-iucn.mjs`, kategori publik nyata, provenance curated vs api). Snapshot data nyata `lib/lab/sample-leads.ts` (`build-leads --emit-sample`); `getScoredLeads()` fallback DB->sample (dilabel CONTOH) jadi dashboard selalu terisi. Dashboard `app/lab/page.tsx` + client `LabLeadsBoard` (urut/filter/cari + rincian skor per sinyal + tautan GBIF/iNat/IUCN + tombol "Promosikan ke Misi" disabled=seam Fase 3). Dev bypass `LAB_DEV_BYPASS` (mustahil di produksi: NODE_ENV+flag .env.local) untuk screenshot /lab tanpa sesi admin. Verifikasi browser: skor membedakan benar (Zaglossus 93/Cyornis ruckii 92 vs harimau Jawa 6), desktop+mobile 360px bersih. Gate hijau (tsc/lint/check:editorial/check:lab-isolation/build). Berikutnya 3.4 promosi lead->Misi. Blocker run nyata kini OPSIONAL (ada fallback). Sebelumnya: Bucket C Step 3.2 Lab harvesters (push `8dc01f5`). Harvester READ-ONLY (GBIF+iNat tanpa auth, IUCN v4 butuh token=tugas founder) tulis hanya ke `content/lab-raw/` (gitignored); satu langkah privileged `build-leads.mjs` upsert ke `lab_leads` pakai service-role key (fail-closed). `scripts/lab/`: `_shared`, `seed-taxa` (12 taksa Lazarus NaLI + still-lost, bukan scan buta), `harvest-gbif` (histogram tahun -> last record/gap/count/dekade), `harvest-inat` (research-grade, match nama ilmiah EKSAK), `harvest-iucn` (no-op tanpa IUCN_API_TOKEN), `build-leads` (signals=bukti mentah ternormalisasi+sumber, TANPA score=Step 3.3; upsert tak menimpa status/notes; --dry-run tanpa key). Bug saat dry-run: iNat fuzzy-match nempel 7734 obs taksa lain ke Gallirallus sharpei -> diperketat eksak (false-negative > false-positive). Dry-run nyata 12 taksa jujur (Zaglossus gap 65thn, Varanus zugorum 46thn, Cyornis ruckii/Gallirallus sharpei 0 rekaman=lost sejati vs refound dgn rekaman terbaru). `.env.local` di-scaffold placeholder SUPABASE_SERVICE_ROLE_KEY+IUCN_API_TOKEN (kosong; founder isi). npm scripts lab:harvest:*+lab:build-leads. Gate hijau (tsc/lint/check:editorial/check:lab-isolation/build). Berikutnya (tunggu approval): Step 3.3 Lazarus Score transparan + dashboard /lab, lalu 3.4 promosi lead->Misi, 3.5 Ghost Signals. Lihat [[reader-intelligence-roadmap]] + `docs/lab-architecture.md`. Sebelumnya: PHASE 2 (Bucket B) LENGKAP -> Checkpoint 2. Step 2.2 Living Articles (push `61906ac`): field author-declared `articleStatus`/`lastVerified`/`supersededBy` (currency TAK ditebak dari umur, anti-spekulasi), isi seam Fase 2 di KnowledgeGenome + section `#riwayat` menyatukan changelog+koreksi; backward-compatible. Step 2.3 Citizen Science Bridge (push `02dfa33`): migrasi 0008 `citizen_reports` (anon insert-only + CHECK, admin via is_admin) + bucket `citizen-reports` (5MB) via MCP advisor bersih; `/api/report` (same-origin/validasi/honeypot/GPS/upload foto), `ReportForm` di /misi (Geolocation opt-in + foto), `/admin/reports` triase + PHASE-3 SEAM untuk Misi Verifikasi Lapangan; **laporan PRIVAT sampai dipromosikan manual** (anti poaching/PII; observasi belum terverifikasi != klaim). Diverifikasi live E2E lalu dibersihkan. Selaraskan detect-contradictions ke `gemini-embedding-001` (push `b51a628`). Flag: `tmp_ingest_embedding` anon-executable SECURITY DEFINER (chip founder). Blocker tunggal tersisa: `GOOGLE_GENERATIVE_AI_API_KEY` (tugas founder) untuk jawaban RAG + `detect:contradictions`. Sisa roadmap = hanya Bucket C (Internal Lab ter-autentikasi; seam Fase 3 sudah terpasang). Lihat [[reader-intelligence-roadmap]]. Gate hijau (tsc/lint/check:editorial/build). Sebelumnya (28 Juni 2026 lanjutan): Bucket A surfacing + Contradiction Engine (Step 2.1) dari blueprint "NaLI Intelligence Platform" (dekomposisi jujur: mayoritas fitur sudah ada; crawler/ML/OSINT diparkir karena risiko langgar anti-spekulasi). Bucket A live (push `36078bd`+merge `3980be1`): 1.1 Tanya NaLI drawer RAG per-artikel (hook bersama `useNaliChat`, fallback stream-kosong, z-index di atas nav), 1.2 Knowledge Genome card (strand claimLedger + seam Fase 2, serap banner Basis tulisan), 1.3 Jalur Baca di /peta-eksplorasi dari SERIES (seam Fase 3). Bucket B 2.1 Contradiction Engine live (push `d0411a6`): migrasi 0007 contradictions (RLS confirmed-only + is_admin, via Supabase MCP), `scripts/detect-contradictions.mjs` (embed claim, cosine>0.82, adjudikasi Gemini, upsert candidate idempotent), `/admin/contradictions` gerbang review manusia, surfacing ke seam KnowledgeGenome+ArticleTutor+section #kontradiksi (verifikasi live via seed lalu hapus). Engine belum hasilkan apa-apa sampai key Gemini diset + detect jalan + kandidat dikonfirmasi (surfacing kosong = aman). Lihat [[reader-intelligence-roadmap]]. Gate hijau (tsc/lint/check:editorial/build). Sebelumnya (28 Juni 2026): perbaikan RAG chat: `/api/chat` balas 200 body kosong karena `GOOGLE_GENERATIVE_AI_API_KEY` belum diset di Vercel (AI SDK baca kunci lazy -> streamText kirim 200 lalu error di tengah stream). Fix kode: guard 503 JSON + onError logger di `app/api/chat/route.ts` (klien `useNaliChat.ts` sudah tangani body kosong dengan anggun, celah murni server). Temuan via Supabase MCP: migrasi 0006 embeddings TAK PERNAH diterapkan ke prod `nali-field-journal` (tabel content_embeddings/RPC match_embeddings/ekstensi vector semua hilang) -> migrasi kuterapkan ke prod (DDL aditif; verifikasi tabel ada 0 baris, 2 RPC, vector+pg_trgm, 1 RLS). Bug migrasi diperbaiki: 0006 pakai gin_trgm_ops tanpa enable pg_trgm -> ditambah + schema-qualify. Sisa tugas founder (kredensial): set GOOGLE_GENERATIVE_AI_API_KEY di Vercel + .env.local, jalankan `npm run ingest` untuk isi embeddings, redeploy. Gate hijau (tsc/lint/check:editorial/build), push main `5e4783e`. Sebelumnya (23 Juni 2026): tiga artikel "alam murni" deep-research global (atas arahan founder, di luar fokus Indonesia, tiap artikel tetap berjangkar Nusantara jujur): #45 Biosfer Gelap (mikroba kerak Bumi; koreksi "10.000 thn"=terbatas, Desulforudis=Chivian 2008), #46 LLSVP/Theia (koreksi paper Theia=Yuan Nature 2023 bukan 2021; vulkanisme RI=subduksi bukan plume), #47 Lautan Canfield (euksinia Permian-Trias; "lautan ungu"=terbatas, "H2S pembunuh tunggal"=diperdebatkan). Aturan baru founder: auto-push ke main tiap build hijau tanpa tanya ([[auto-push-after-build]]). Gotcha: diagram SVG harus di-inline `![](src)` di body agar render sebagai figure (kalau tidak, check:article-images gagal). Gate semua hijau (tsc/check:editorial/build/check:article-images 47/47), push `dfdc26a` lalu `b864e8d`. Total artikel = 47. Sebelumnya (22 Juni 2026): dua artikel sejarah deep-research (Contradiction Matrix sumbu metodologi). Samalas 1257 diperdalam (perbaikan akurasi kausalitas kelaparan 1258 via Guillet 2017; friksi model vs lingkar pohon via Wade 2020; klaim London = diperdebatkan, Pamatan = belum cukup bukti; push `38066ed`) + artikel BARU Selat Muria/Demak/Bledug Kuwu (narasi populer vs geomorfologi Sunarto 2008; hoaks "selat kembali 2024" diluruskan; air asin Bledug Kuwu dari sedimen Kendeng lebih tua, bukan dasar selat Holosen; 5 source file + 2 foto CC BY-SA + peta SVG; push `b70d46d`). Prompt "ABYSS-WALKER" (500 sumber fiktif + struktur konspirasi) ditolak; founder setuju, arahkan ke riset asli. Gate hijau (tsc/check:editorial/check:article-images 44/44/build), produksi terverifikasi. Total artikel = 44. Sebelumnya (16 Juni 2026): klaster Lazarus Wave 3 C3 capstone "Spesies Indonesia yang Masih Hilang" terbit (sintesis taksa belum ditemukan: sikatan Rueck/biawak Zug/mandar Sharpe; definisi hilang vs punah; "sudah punah" = belum cukup bukti; menaut ke /bukti-dicari + /misi; foto spesimen CC BY 3.0). KLASTER LAZARUS PETA AWAL LENGKAP (Wave 1-3). Gate hijau (tsc/check:editorial/check:article-images 42/42/build), produksi terverifikasi, push `15e85ef`. Total artikel = 42. Sebelumnya: klaster Lazarus Wave 3 C2 Nepenthes pitopangii terbit (kantong semar Sulawesi yang lama hanya diketahui dari satu individu, populasi kedua ditemukan 2011; mendiversifikasi klaster ke tumbuhan; confidence high, 4 sumber, foto CC BY 2.0 Alfindra Primaldhi; ditandai jujur bukan Lazarus klasik). Gotcha: sumber buku butuh url bibliografi yang dapat ditelusuri agar lolos check:editorial. Gate hijau (tsc/check:editorial/check:article-images 41/41/build), produksi terverifikasi, push `99ff8dc`. Total artikel = 41. Sebelumnya: klaster Lazarus Wave 3 C1 seriwang Sangihe terbit (Eutrichomyias rowleyi, hilang 125 thn ditemukan 1998, kini terancam tambang emas; confidence high, 4 sumber, foto hidup CC0 James Eaton, ancaman tambang ditandai diperdebatkan). Gate hijau (tsc/lint/check:editorial/check:article-images 40/40/build), produksi terverifikasi, push `a58b723`. Total artikel = 40. Sebelumnya: klaster Lazarus Wave 2: B2 lebah raksasa Wallace terbit (lebah terbesar dunia, hilang 38 thn, ditemukan 2019 Halmahera; confidence high, 4 sumber, ilustrasi Smith 1860 domain publik) + B1 coelacanth ditingkatkan ke standar klaster (claim ledger spesifik, related, internalScore). Ketiga artikel Wave (A4/B1/B2) diperluas lewat floor 900 kata. Gotcha: server node basi di :3000 bikin curl lokal kena build lama (cek `lsof :3000`, kill, start ulang). Gate hijau (tsc/lint/check:editorial/check:article-images/build), produksi terverifikasi, push `e9b1844`. Total artikel = 39. Sebelumnya: klaster Lazarus A4 black-browed babbler terbit (burung hilang 172 tahun, ditemukan dua warga Kalsel; confidence high, 4 sumber, foto burung hidup CC BY 4.0; Wave 1 A1-A4 selesai). Backlog riset dibersihkan dari 3 entri yang kini terbit. Gotcha: push lewat SSH-over-443 karena port 22 diblokir jaringan. Semua gate hijau (tsc/lint/check:editorial/check:article-images/build), produksi terverifikasi, push `c8667e8`. Sebelumnya: beranda V2 (`app/page.tsx`) kini jadi ruang kendali: LivingDashboard tampil tepat di bawah hero + grid "Modul V2" menautkan 8 surface (ruang-kendali/linimasa/peta-indonesia/koneksi/bukti-dicari/misi/aktivitas/banding); section lama (apa-ini, tiga pilar, tulisan terbaru, callout arsip) dipertahankan. Sebelumnya: NaLI V2 Modul 2-12 dieksekusi additive (rute baru /linimasa, /bukti-dicari, /aktivitas, /koneksi, /banding, /misi, /peta-indonesia + Discovery Score di /ruang-kendali + archive pipeline scaffold loc.gov; semua dari data nyata, tanpa dep baru). Gate hijau (tsc/lint/check:editorial/build). Lihat `NALI_V2_LIVING_ENGINE.md`. Sebelumnya: NaLI V2 Modul 1 Living Knowledge Engine (dashboard `/ruang-kendali` dari data nyata, RSC+Tailwind tanpa dep baru, beranda lama utuh; satu modul per sesi per LARANGAN-V2-002). Gate hijau (tsc/lint/check:editorial/build). Lihat `NALI_V2_LIVING_ENGINE.md`. Sebelumnya: Fase 7 knowledge pipeline (harvest OpenAlex nyata -> +306 jurnal & +306 arsip sumber tertaut, jurnal=330/arsip=465 lewat target 300; artikel/investigasi tetap editorial deep-research, bukan massal). Semua gate hijau (tsc/lint/check:editorial/build). Lihat `docs/nali_fase7_knowledge_pipeline.md` + `content/logs/progress.json`. Sebelumnya: eksekusi NALI_MASTER_BUILD penuh Langkah 1-11 (bug fix, audit + perbaikan 13 URL mati, global search Cmd+K, SEO/RSS, claim ledger/related/depth, newsletter API + series nav + peta eksplorasi interaktif, sitasi + advanced arsip search + /topik, koreksi/catatan-backlog/changelog, internalScore + artikel Pesut Mahakam hasil deep-research). Semua gate hijau, produksi terverifikasi. Pushed `fa14264`. Lihat `NALI_BUILD_REPORT_2026-06-15.md`.*
