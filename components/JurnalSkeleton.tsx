...
export function JurnalSkeleton() {
  return (
    <div className="grid lg:grid-cols-[320px_1fr] gap-8 items-start relative">
      {/* Sidebar Placeholder */}
      <aside className="hidden lg:block border border-dashed border-ink/40 bg-ink-wash/10 p-5 space-y-6 lg:sticky lg:top-24 h-[600px] animate-pulse">
      ...
      <div className="space-y-5 flex-1">
        <div className="hidden lg:flex items-center justify-between border-b border-dashed border-ink/20 pb-3">
        ...
        <div className="grid gap-5 grid-cols-1 [@media(min-width:1400px)]:grid-cols-2">
          {[...Array(4)].map((_, i) => (
            <div
              key={i}
              className="animate-pulse border border-dashed border-ink/40 bg-paper flex flex-col md:flex-row"
            >
              {/* Cover Placeholder matching responsive sizes */}
              <div className="w-[140px] sm:w-[160px] md:w-[180px] lg:w-[200px] xl:w-[220px] aspect-[3/4] bg-ink-wash/20 border-b md:border-b-0 md:border-r border-dashed border-ink/30 flex-shrink-0" />
