...
/**
 * Drifting noise-driven particles. Slate-tinted to NaLI's Arsip Sumber accent;
 * dark slate on light paper, light slate on dark. Renders a transparent canvas
...
export const FluidParticlesBackground = ({
  ...
  useEffect(() => {
    ...
    const animate = () => {
      ...
      // fade the previous frame toward transparent → trails on a clear canvas
      ctx.globalCompositeOperation = "destination-out";
      ctx.fillStyle = "rgba(0,0,0,0.06)";
      ctx.fillRect(0, 0, canvas.width, canvas.height);
      ctx.globalCompositeOperation = "source-over";

      // slate tint synced to the Arsip Sumber theme
      const [pr, pg, pb] = isDark ? [156, 180, 202] : [66, 92, 116];
      ...
      for (const particle of particles) {
        ...
        const opacity = Math.sin((particle.life / particle.maxLife) * Math.PI) * 0.34;
