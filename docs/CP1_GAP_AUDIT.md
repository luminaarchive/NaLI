...
Sprint 0 through Sprint 0.7 has the core paid export foundation in place: report generation, markdown export, PDF export, feedback capture, integrity guardrails, payment-gated unlock, a Midtrans Snap code path, minimal founder admin visibility, CP1-safe `report_events`, CP1-safe `api_usage_logs`, and at least three public report templates. The remaining CP1 blocker is production Midtrans activation: Vercel Production does not currently have the required Midtrans env values, so production readiness correctly reports Midtrans as not configured and manual pending payment remains fallback only.
...
| Midtrans automatic code path        | READY IN CODE          | `/api/payments/create` creates pending rows and can request Snap checkout when server-only Midtrans env is configured; webhook validation accepts only valid successful Midtrans states.                                                     | Keep tests and smoke coverage; do not claim production automatic checkout until env is active.                    |
| Midtrans production env             | BLOCKED BY ENV         | Production readiness currently reports `midtransConfigured: false`, so automatic checkout is not active.                                                                                                                                    | Add server-only Midtrans env in Vercel, set Midtrans webhook URL, redeploy, then run `npm run smoke:export:prod`. |
| Manual payment fallback             | DONE / FALLBACK ONLY   | Missing Midtrans env returns honest manual pending mode, creates a real pending payment row, and does not fake success.                                                                                                                      | Use only as fallback until automatic checkout is verified.                                                        |
...
| Basic templates min. 3              | DONE           | `reportTemplates` includes `Laporan Praktikum Biologi`, `Laporan Observasi Lingkungan`, and `Laporan Kegiatan/KKN` plus additional basic Learn & Report templates.                                                                          | Do not add large template infrastructure now.                                                                     |
...
| `report_events` audit logging       | DONE           | Sprint 0.7 adds additive `report_events` migration, sanitized server-only logger, readiness count, and minimal founder count.                                                                                                                | Keep event metadata sanitized and non-blocking.                                                                   |
| `api_usage_logs` cost logging       | DONE           | Sprint 0.7 adds additive `api_usage_logs` migration, internal aliases, readiness count, and skipped/failed operation count. Provider/model/cost details are not exposed in public UI.                                                       | Keep aliases internal; leave exact cost null when unavailable.                                                    |
| Minimal cost logging                | DONE           | `usage_events` remains backward-compatible and now also feeds CP1-safe `api_usage_logs`.                                                                                                                                                    | Keep logs non-blocking and avoid provider-cost public language.                                                   |
...
CP2 remains blocked until CP1 is live with a real verified Midtrans transaction. Do not build upload pipeline, Literature Matrix automation, source resolvers, Professional Dashboard, PostGIS/H3, SOS, DOCX, subscription, or other CP2+ surfaces before that trigger.
