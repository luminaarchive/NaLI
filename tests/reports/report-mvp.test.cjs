...
test("public homepage uses final Indonesian Sprint 0 hero and query box copy", () => {
  const homepage = fs.readFileSync(path.join(repoRoot, "src/app/page.tsx"), "utf8");
  const commandBox = fs.readFileSync(path.join(repoRoot, "src/components/report/HomeCommandBox.tsx"), "utf8");
  const source = `${homepage}\n${commandBox}`;

  assert.match(source, /Ubah catatan menjadi laporan berbasis bukti\./);
  assert.match(source, /Tulis catatan, topik, lokasi, atau sumbermu/);
  assert.match(source, /Mulai Laporan/);
  assert.match(source, /NaLI untuk Kerja/);
  assert.match(source, /NaLI menyusun, bukan mengarang/);
  assert.doesNotMatch(source, /Evidence-based AI|Start a report|Explore NaLI for work|Export Gate: Active/i);
});

test("homepage query box supports examples and create-report prefill intent", () => {
  const commandBox = fs.readFileSync(path.join(repoRoot, "src/components/report/HomeCommandBox.tsx"), "utf8");

  assert.match(commandBox, /Saya mengamati erosi di Banjir Kanal Semarang/);
  assert.match(commandBox, /Aku mau laporan observasi sungai tapi belum punya catatan/);
  assert.match(commandBox, /Saya punya hasil praktikum biologi, bantu susun laporan/);
  assert.match(commandBox, /belum punya bahan/);
  assert.match(commandBox, /start_from_zero/);
  assert.match(commandBox, /draft_from_materials/);
  assert.match(commandBox, /nali-create-report-prefill/);
});

test("create-report page is Indonesian-first and accepts homepage query params", () => {
  const page = fs.readFileSync(path.join(repoRoot, "src/app/create-report/page.tsx"), "utf8");
  const form = fs.readFileSync(path.join(repoRoot, "src/components/report/CreateReportForm.tsx"), "utf8");
  const source = `${page}\n${form}`;

  assert.match(source, /Mulai Laporan/);
  assert.match(source, /Masukkan bahan yang kamu punya, atau mulai dari nol/);
  assert.match(source, /params\.get\("q"\)/);
  assert.match(source, /params\.get\("template"\)/);
  assert.match(source, /Upload PDF belum aktif di environment ini/);
  assert.doesNotMatch(source, /Start Building a Report|Enter your materials|0 bahan terisi/i);
});

test("pricing page is Sprint 0 one-time/export framed, not subscription-first", () => {
  const pricing = fs.readFileSync(path.join(repoRoot, "src/app/pricing/page.tsx"), "utf8");

  assert.match(pricing, /Harga Sprint 0/);
  assert.match(pricing, /Payment gateway belum aktif di MVP ini/);
  assert.match(pricing, /Preview Gratis/);
  assert.match(pricing, /One-Time Report/);
  assert.match(pricing, /Export Premium/);
  assert.match(pricing, /NaLI Energy Pack/);
  assert.doesNotMatch(pricing, /\/mo|monthly subscription|checkout.*live|payment.*active/i);
});

test("Field Intelligence page is clearly future-positioned and not fake realtime", () => {
  const field = fs.readFileSync(path.join(repoRoot, "src/app/field-intelligence/page.tsx"), "utf8");

  assert.match(field, /Dibangun bertahap/);
  assert.match(field, /belum aktif penuh di MVP publik/);
  assert.match(field, /Catatan lapangan/);
  assert.match(field, /Review manusia/);
  assert.match(field, /Export evidence/);
  assert.doesNotMatch(field, /fake maps|realtime|verified records|live field data|operational system/i);
});

test("result page frames export as locked premium preview without active payment claims", () => {
  const resultClient = fs.readFileSync(path.join(repoRoot, "src/components/report/ReportResultClient.tsx"), "utf8");

  assert.match(resultClient, /Preview laporan\/panduan/);
  assert.match(resultClient, /Export Premium/);
  assert.match(resultClient, /Preview ini bisa dibaca gratis/);
  assert.match(resultClient, /Export premium terkunci/);
  assert.match(resultClient, /Salin preview/);
  assert.doesNotMatch(resultClient, /Unlock Export|Export Gate Active|Payment Active|Checkout available/i);
});

test("public navigation stays minimal and never links internal system routes", () => {
  const navSource = [
    "src/components/ui/SiteNav.tsx",
    "src/components/ui/CodexNav.tsx",
    "src/app/page.tsx",
  ]
    .map((file) => fs.readFileSync(path.join(repoRoot, file), "utf8"))
    .join("\n");

  assert.match(navSource, /Learn & Report/);
  assert.match(navSource, /Field Intelligence/);
  assert.match(navSource, /Harga/);
  assert.doesNotMatch(navSource, /href="\/system|href="\/api\/system|\/system\/orders|\/api\/maintenance\/purge/);
});
