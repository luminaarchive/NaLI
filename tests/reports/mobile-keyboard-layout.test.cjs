...
test("AgentWorkspace tracks composer focus state and adjusts bottom padding dynamically", () => {
  ...
  assert.match(
    code,
    /flex flex-wrap gap-2 py-1 justify-center/,
    "Bottom composer suggested actions container should wrap flex items"
  );
  assert.match(
    code,
    /min-h-\[44px\]/,
    "Suggested action buttons must keep a minimum touch target height of 44px"
  );
  ...
  assert.match(
    ...
    "Model selector buttons container should wrap and be compact"
  );
  assert.match(
    code,
    /flex-1 sm:flex-none/,
  ...
  assert.strictEqual(
    code.includes("midtransCheckoutActive = true"),
    false,
    "Midtrans/payments must not be activated"
  );
